...
- SQLite 持久化队列替代内存队列（`infrastructure/queue/persistent_queue.py`）
...
- SLM 输出结构固定：`{"keep": bool, "importance": float, "confidence": float, "type": str, "tag": str, "summary": str, "summaries": [str]}`
- Prompt 版本化（当前 v3.0）
- 类型体系：`ENTITY`（实体）/ `RELATION`（关系）/ `EVENT`（事件）/ `TASK`（任务）
- 标签体系：`identity` / `preference` / `project` / `fact` / `task` / `knowledge` / `noise`
- 决策矩阵（importance × confidence 四象限）：
  - `importance ≥ 0.7 + confidence ≥ 0.7` → golden（黄金记忆）
  - `importance ≥ 0.7 + confidence < 0.7` → review（风险记忆）
  - `importances < 0.7 + confidence ≥ 0.7` → low（低优先）
...
```json
{
  ...
  "session_id": "s-xxx",
  "turn_id": "abc123",
  "source_user": "我喜欢用Python做AI",
  "source_assistant": "Python很适合AI开发",
  "timestamp": 1234567890
}
```
...
```python
# rag-api/infrastructure/config/config.py
MEMORY_LAYERS = {
    "general": "默认对话",
    "story": "故事创作与世界观设计",
    "docreader": "文档分析与阅读",
