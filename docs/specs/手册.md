...
修改 `rag-api/infrastructure/config/config.py` 中的 `MEMORY_LAYERS` 字典：
...
涉及的文件：
- `rag-api/infrastructure/config/config.py` — 定义层名和说明
- `rag-api/infrastructure/runtime/state.py` — 切换时校验层名（自动读取 Config，无需改动）
- `rag-api/infrastructure/vector/qdrant_store.py` — 存储/搜索时按 layer 过滤（无需改动）

---

### 模型路由使用规范
- 单模型调用：直接传入模型 ID（例如 'deepseek-r1:8b'），层级自动设为 'general'。
- 记忆层调用：直接传入层级名称（例如 'story'），自动走默认 DeepSeek 模型。
- 组合模式：使用 '模型:层级' 格式（例如 'deepseek-r1:8b:story'）。
- 默认兜底：系统会自动将复杂层级（story/docreader）强制路由至 DeepSeek 增强推理能力。
...
1. 修改 `rag-api/infrastructure/config/config.py` 中的 `LLM_PROVIDER`
