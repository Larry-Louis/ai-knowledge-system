...
POST /v1/chat/completions
  ...
  |
  |- [S0-2]  会话 ID 推导                    rag-api/core/memory.py:58
  |
  |- [S0-3]  用户消息向量化                   rag-api/services/embedding.py:7
  |
  |- [S0-4]  三重记忆检索                     rag-api/services/qdrant_store.py
  |
  |- [S0-5]  记忆合并去重                     rag-api/core/memory.py:39
  |
  |- [S0-6]  摘要检索 + 文档检索               rag-api/services/qdrant_store.py
  |
  |- [S0-7]  核心写入触发（可选）              rag-api/core/memory.py:112
  |
  |- [S0-8]  构建 RAG 提示                    rag-api/core/prompt.py:6
  |
  |- [S0-9]  LLM 调用                         rag-api/core/llm.py
  |
  |- [S0-10] 同步写入：用户消息 -> Qdrant      rag-api/core/memory.py:152
  |
  |- [S0-11] 同步写入：AI响应 -> Qdrant        rag-api/core/memory.py:165
  |
  |- [S0-12] 提交异步管道任务                  rag-api/core/memory_pipeline.py:54
  |
  |- [S0-13] 条件性摘要生成                    rag-api/core/memory.py:188
  |
  +- HTTP 200
```
...
| 查询    | 函数                             | 行号  | 参数                                                       |
| ----- | ------------------------------ | --- | -------------------------------------------------------- |
| 会话记忆  | `search_memories()`            | 74  | 仅当前 `session_id`，`type=memory`，top_k=8                   |
| 全局记忆  | `search_global_memories()`     | 101 | 按 `layer IN [core, active_role]` 过滤，top_k=6，core 层 x1.05 |
| 近期跨会话 | `get_recent_global_memories()` | 128 | **仅新会话触发**，排除当前 `session_id`，每角色层最多 2 条                  |
...
**摘要检索：** `get_summary()` (`rag-api/services/qdrant_store.py:192`)
- 滚动查询 `type=summary` 的最新记录
- 返回当前世界观摘要文本或 None

**文档检索：** `search_documents()` (`rag-api/services/qdrant_store.py:167`)
...
### [S0-10] [S0-11] 同步写入 Qdrant -- `rag-api/core/memory.py:152,165`
...
**过滤：** `_is_auto_task()` (`rag-api/core/memory.py:10`) 检测 Open WebUI 自动生成的任务消息并跳过
...
```
_worker() 守护线程
  |  (模块导入时启动, rag-api/core/memory_pipeline.py:157)
  |
...
### [S1-0] 后台线程启动 -- `rag-api/core/memory_pipeline.py:157`
...
### [S1-1] 崩溃恢复 -- `rag-api/services/persistent_queue.py:109`
...
### [S1-2] 定期清理 -- `rag-api/services/persistent_queue.py:127`
...
### [S1-3] 出队 -- `rag-api/services/persistent_queue.py:65`
...
### [S1-4] 核心处理 -- `rag-api/core/memory_pipeline.py:118`
...
#### [S1-4-Rule] 综合得分评估 -- `rag-api/core/rule_evaluator.py`
...
### [S1-4b] SLM 验证 -- `rag-api/core/text_utils.py:69`
...
**输出解析：** `_safe_parse_json()` (`rag-api/core/text_utils.py:58`)
- 支持多种容错解析：纯 JSON、markdown fence 包裹、正则提取、关键词兜底

---

### [S1-4c] 决策矩阵 -- `rag-api/core/decision_maker.py:17`
...
### [S1-4d] 回退提取 -- `rag-api/core/text_utils.py:54`
...
### [S1-4e] 存储 Memory Unit -- `rag-api/core/memory_pipeline.py:99`

**函数：** `_store_mu()`

每个摘要最终通过四步清洗后写入 Qdrant：

**第一步 标准化：** `normalize()` (`rag-api/core/text_utils.py:12`)
- 主语统一：`"我"` -> `"用户"`，`"我们"` -> `"用户"`
- 术语标准化：大小写统一（autosar -> AUTOSAR, rag -> RAG, python -> Python 等）
- 15 个预置技术名词映射

**第二步 去重：** `is_duplicate()` (`rag-api/core/text_utils.py:43`)
- 对内容做 embedding（调用 Ollama）
- 在 Qdrant `memory_unit` 类型中搜索 Top 5
- **余弦相似度 >= 0.90 -> 视为重复，跳过存储**

**第三步 极性冲突检测与解决：** `detect_polarity()` (`rag-api/core/text_utils.py:29`)
- 基于关键词计数判断极性：
  - 正面词：喜欢、爱、支持、推荐...
  - 负面词：不喜欢、讨厌、拒绝、无法...
...
### [S1-5] 标记完成/失败 -- `rag-api/services/persistent_queue.py:83,92`
...
| 文件 | 职责 |
|------|------|
| `rag-api/api/chat.py` | HTTP 入口，解析请求，构建响应 |
| `rag-api/core/memory.py` | MemoryManager：阶段 0 的核心编排 |
| `rag-api/core/memory_pipeline.py` | 阶段 1 的全部逻辑（worker、SLM、MU处理） |
| `rag-api/core/prompt.py` | RAG 提示构建 |
| `rag-api/core/llm.py` | LLM 客户端工厂 |
| `rag-api/core/config.py` | 全局配置 |
| `rag-api/core/state.py` | 运行时状态（活跃角色、核心写模式、活跃文档） |
| `rag-api/services/embedding.py` | 向量化服务（Ollama nomic-embed-text） |
| `rag-api/services/qdrant_store.py` | Qdrant 数据访问层 |
| `rag-api/services/session_store.py` | 内存会话存储 |
| `rag-api/services/persistent_queue.py` | SQLite 持久队列 |
